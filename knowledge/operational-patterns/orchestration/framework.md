...
**CC primitive**: The `agent` setting in `settings.json` runs the main thread as a named subagent definition from agent files. This is a generic CC mechanism — it accepts any agent name, not a built-in role keyword. CC discovers agents from `agents/` (plugin root), `.claude/agents/` (standalone), or `~/.claude/agents/` (personal).

**CAB convention**: Create an `orchestrator.md` agent definition (in `~/.claude/agents/` for global, or `agents/` for plugin projects) and set `"agent": "orchestrator"` in `settings.json` to establish the orchestrator pattern below. The "orchestrator" name is a CAB convention, not a CC built-in.

```jsonc
// ~/.claude/settings.json — CAB orchestrator convention
{ "agent": "orchestrator" }  // References agents/orchestrator.md (plugin) or .claude/agents/orchestrator.md (standalone)
```
