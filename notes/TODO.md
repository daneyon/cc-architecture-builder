...
- [ ] **CAB commands archive validation** (Q2 surface, Session 41) — user has sunset commands from their personal usage; CAB's `commands/` dir contains shims (e.g., `/cab:execute-task` → invokes `execute-task` skill). Validate: are any commands NOT shimmed-to-skill (i.e., genuine command-only logic)? If all are shims, candidate for archive in Wave 9+ cleanup. Use `audit-workspace` Dimension 8 + 5-axis audit framework once consumer-skill updates land (Phase 2E.5+).
- [ ] **knowledge/INDEX.md full regen via cab:index-kb** (Session 41 settings hardening surface, 2026-04-30) — drift detected: root INDEX showed `reference/: 1` (actual 7), `operational-patterns/: 12` (actual 16); sub-INDEX `operational-patterns/INDEX.md` missing 2 pre-existing files (`cc-memory-layer-alignment.md` + `multi-agent/agent-resolution.md`). Session 41 commit surgically patched operational-patterns root + sub INDEX entries for new card; remaining drift untouched. Full regen via `cab:index-kb` skill in dedicated session — preserves manual editorial sections while updating counts + missing entries.
- [ ] **LL-30 + LL-31 formal codification** (Session 41 surface, 2026-04-30) — Multiple Session 41 artifacts (security.md 6th bullet, component-decision-framework.md, cross-project-settings-hardening.md, design-principles.md DP8) reference LL-30 ("DP8 enforcement gap; passive-doc insufficient") and LL-31 ("default-deny on settings.json edits"). Neither entry currently exists in `notes/lessons-learned.md`. Promote both at next lessons-learned review pass to resolve orphan references. Memory refs: existing LL-30 implicit context across multiple files; new LL-31 candidate captured in `feedback_settings_json_default_deny_edit.md`.
