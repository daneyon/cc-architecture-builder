...
**Last Updated**: 2026-04-30 (Session 41 — Phase 2B'.4 + Phase 2E.1-2.2 complete; 2B'.5 placeholder staged; 2B'.6-8 + 2E.3-6 pending)
...
- `notes/impl-plan-kb-to-kg-2026-04-28-v2.md` (UXL-005; Phase 2A ✓ + 2B'.1-4 ✓ + Phase 2E.1-2 ✓ in `56e8d34`; 2B'.5 placeholder + 2B'.6-8 + 2E.3-6 pending)
**Active Task**: User to author temporal-neutrality rule (2B'.5 placeholder in `.claude/rules/kb-conventions.md`); then resume 2B'.6-8 + Phase 2E.3-6 — see `notes/current-task.md`
...
- [ ] **Phase 2E.5+ consumer-skill updates** (load-bearing back-check from 2E.2 framework card cross-refs, Session 41) — `audit-workspace` Dimension 8 needs to consume `component-decision-framework.md` matrix + anti-patterns as its scoring rubric; `scaffold-project` Step 0 needs to traverse the decision tree at scaffolding time + default-include 3-dir bundled-resources placeholders (per `feedback_skill_default_placeholder_bundled_resources` memory); `integrate-existing` needs to apply framework when overlaying CC architecture; `architecture-advisor` agent needs framework cross-ref. Forward-statement currently in framework card's See Also; back-check tracked here.
- [ ] **Wave 9+: New KB card `knowledge/operational-patterns/cross-platform-interop.md`** (Q3 surface, Session 41) — strategic guide on MCP-as-canonical-interop primitive for agentic-OS platforms (vs. traditional API vs Plugin); covers HydroCast↔RAS-exec/GTA tooling exchange pattern; bidirectional MCP composition; security via `userConfig` `sensitive: true`; bridge-package anti-pattern. Forward-pointer in Worked Example #4 of `component-decision-framework.md`.
- [ ] **Wave 9+: New KB card `knowledge/operational-patterns/rag-scaling-decision-framework.md`** (Q3 surface, Session 41) — RAG scaling decision framework for KBs at 100+ files threshold (HydroCast / RAS-exec / GTA likely candidates). Currently loosely standardized as CAB advisory; needs systematization via Wave 8 KG work. Could absorb into 2F schema design OR land separately at Wave 9.
- [ ] **Pre-Wave-10: SCHEDULE-feature integration** (Q2 surface, Session 41) — the new CC schedule feature (`/loop`, routines, desktop tasks, github actions) is potentially load-bearing for state-mgmt automation: periodic LL→UXL triage, scheduled audit-workspace runs, recurring KB freshness checks (LL-10), automated state-refresh on long-running tasks. Design integration into CAB skills/operational-patterns BEFORE Wave 10 so state-mgmt-related ops can leverage. Specific candidates: `triage-lessons` skill (uses `/loop` for periodic promotion); `kb-freshness-check` skill (uses routines for daily review_by-overdue scan); HydroCast PR-watch routine. Cross-ref: `knowledge/components/component-decision-framework.md` § HOOK vs SCHEDULE table.
- [ ] **DP10 candidate: Backend/Artifact-First Architecture** (Q4 surface, Session 41) — user's load-bearing design philosophy (pipeline → outputs → API → GUI; viz is rendering not authoring; canonical structured data is source-of-truth) is currently captured as memory `project_backend_first_architecture_philosophy.md` + anti-pattern in `component-decision-framework.md`. Promotion candidate for next CAB design-principles roster review (Wave 9+ or Wave 11+ consolidation pass). Decide: standalone DP10 or extend DP3 (Standardized KB).
- [ ] **CAB commands sunset + archive** (UPGRADED P1, Session 42 surface 2026-05-01) — user has sunset commands from personal usage AND empirical evidence in Session 42 confirmed type-shadowing failure mode: Skill tool resolved `cab:execute-task` to commands/execute-task.md shim instead of skills/execute-task/SKILL.md, contradicting DP2 "skills win" precedence. Action plan: (1) fresh-fetch CC docs on skill `user-invocable` default behavior + slash-invocation surface; (2) `head -20` validate all 15 commands are shims; (3) decision gate — if `user-invocable` defaults true, archive `commands/` directly; if false, add `user-invocable: true` to all 15 skill counterparts THEN archive (2-step migration); (4) update plugin.json + KB references + CAB CLAUDE.md; (5) extend `/sync-check` to detect type-shadowing (same-scope command name == skill name). 15 command files at risk: init-worktree, validate, add-agent, add-command, execute-task, add-skill, commit-push-pr, context-sync, kb-index, sync-check, techdebt, init-plugin, integrate-existing, new-global, new-project. Memory: `project_command_skill_type_shadowing.md`.
- [X] **knowledge/INDEX.md full regen via cab:index-kb** — ✓ in `9c04f21` (Session 42, 2026-05-01). Root INDEX `reference/`: 1 → 8 (back-filled 6 prior-relocated cards + new llm-interaction-patterns); `components/`: 10 → 11 (component-decision-framework added as cross-cutting meta-card); `file_count`: 37 → 48. Reference + components sub-INDEXes resynced. Operational-patterns category was already clean (back-filled in `bf174fd`). Verified: 47/47 .md files have `source:`; arithmetic 3+2+3+11+2+8+16+1+2 = 48; idempotent.
- [ ] **LL-30 + LL-31 + LL-32 formal codification** (UPGRADED P1, Session 42 surface 2026-05-01) — three orphan-referenced LL candidates needing formal entries in `notes/lessons-learned.md`: LL-30 ("DP8 enforcement gap; passive-doc insufficient" — Session 41 surface), LL-31 ("default-deny on settings.json edits" — Session 41 surface), LL-32 ("command↔skill type-shadowing at same scope contradicts DP2 'skills win' precedence" — Session 42 empirical surface; distinct from LL-27 scope-shadowing). Cross-references already exist in security.md, component-decision-framework.md, cross-project-settings-hardening.md, design-principles.md DP8 (LL-30/31) and `project_command_skill_type_shadowing.md` memory (LL-32). Codify all three in a single batch pass to resolve orphan references atomically. Each gets its own integration plan; LL-32's integration layer is the commands archive + `/sync-check` extension.
