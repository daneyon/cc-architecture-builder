...
- **Hooks**: PostToolUse on Write|Edit -> `ruff format $CLAUDE_FILE_PATH`, PreToolUse bash-security-gate.sh
- **Sandbox**: enabled (filesystem deny rules, platform note: macOS/Linux/WSL2 only)
- **Permissions**: 32 deny patterns (destructive ops, git force, shell escapes, sensitive paths)
